...
namespace SSMT3
{
    public partial class ModManagePage
    {
        ...
        private void AddModArchiveFile(string archiveFilePath)
        {
            ...
            //新增的Mod必须得是开启的状态，所以这里不需要改成DISABLED
        ...
        private void ListView_ModItem_DoubleTapped(object sender, DoubleTappedRoutedEventArgs e)
        {
            ...
            {
                int selectedIndex = ListView_ModItem.SelectedIndex;
                if (selectedIndex < 0 || selectedIndex >= ModItemList.Count)
                {
                    return;
                }

                ModItem? modItem = ModItemList[selectedIndex];
                if (modItem == null || string.IsNullOrWhiteSpace(modItem.ModName))
                {
                    _ = SSMTMessageHelper.Show("未找到有效的 Mod 信息");
                    return;
                }

                string secondaryModFolderPath = Get_CategorySecondary_ModRepoFolderPath();
                if (string.IsNullOrWhiteSpace(secondaryModFolderPath))
                {
                    _ = SSMTMessageHelper.Show("未找到二级分类路径");
                    return;
                }
                ...
                {
                    _ = SSMTMessageHelper.Show($"Mod 路径不存在: {targetModFolderPath}");
                    return;
                }

                bool isDisabled = modItem.ModName.StartsWith("disabled", StringComparison.OrdinalIgnoreCase);
                string newModFolderName = isDisabled
                    ? modItem.ModName.Substring("disabled".Length)
                    : "DISABLED" + modItem.ModName;

                if (string.IsNullOrWhiteSpace(newModFolderName))
                {
                    _ = SSMTMessageHelper.Show("目标 Mod 名称无效");
                    return;
                }

                string newModFolderPath = Path.Combine(secondaryModFolderPath, newModFolderName + "\\");
                if (Directory.Exists(newModFolderPath))
                {
                    _ = SSMTMessageHelper.Show($"目标目录已存在: {newModFolderPath}");
                    return;
                }
        ...
        private void LoadDirectoryPicturesToFlipView(string imageFolderPath,List<IniResource> ExcludeTextureResourceList)
        {
            try
            {
                ...
                try
                {
                    ...
                    imageFiles = Directory.GetFiles(imageFolderPath, "*.*", System.IO.SearchOption.AllDirectories)
