...
namespace SSMT3
{
    public partial class WorkPage
    {
        ...
        {
            if (Directory.Exists(PathManager.Path_ResourcesFolder))
            {
                SSMTCommandHelper.ShellOpenFolder(PathManager.Path_ResourcesFolder);
            }
        }
        ...
        {
            string LogFilePath = PathManager.Path_LatestSSMTLogFile;
        ...
        private void Menu_ToolBox_CleanFrameAnalysisFolderButKeepLatest_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                string LatestFrameAnalysisFolderName = PathManager.LatestFrameAnalysisFolderName;

                List<string> frameAnalysisFileList = SSMTResourceUtils.GetFrameAnalysisFileNameList();
                foreach (string FrameAnalysisFolderName in frameAnalysisFileList)
                {

                    if (FrameAnalysisFolderName == LatestFrameAnalysisFolderName)
                    {
                        continue;
                    }

                    string TargetFolder = Path.Combine(PathManager.CurrentGameMigotoFolder, FrameAnalysisFolderName);
        ...
        private void Menu_ToolBox_CleanAllFrameAnalysisFolder_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                List<string> frameAnalysisFileList = SSMTResourceUtils.GetFrameAnalysisFileNameList();
